...
class LidarDataset2D(Dataset):
    ...
    def split(self, split_ratio=0.9):
        dataset1 = copy.deepcopy(self)
        dataset1.lidar_data = dataset1.lidar_data[:int(len(self.lidar_data) * split_ratio)]
        dataset1.beam_output = dataset1.beam_output[:int(len(self.beam_output) * split_ratio)]
        dataset2 = copy.deepcopy(self)
        dataset2.lidar_data = dataset2.lidar_data[int(len(self.lidar_data) * split_ratio):]
        dataset2.beam_output = dataset2.beam_output[int(len(self.beam_output) * split_ratio):]
