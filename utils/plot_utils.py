...
def plot_loss_acc_curve(loss_train, loss_val, acc_val, rounds, args):
    ...
    plt.text(6, 2, 'max_top1_acc {:.4f}%'.format(
        100*max_ignore_None([i[0] for i in acc_val])), fontsize=fontsize)
    plt.text(6, 1, 'max_top5_acc {:.4f}%'.format(
        100*max_ignore_None([i[1] for i in acc_val])), fontsize=fontsize)
    plt.text(6, 0, 'max_top10_acc {:.4f}%'.format(
        100*max_ignore_None([i[2] for i in acc_val])), fontsize=fontsize)

    savePath = "./save"
    if args.plot_save_path != "default":
        savePath = args.plot_save_path
    os.makedirs(savePath, exist_ok=True)
    savePath = os.path.join(savePath, 'RoundDuration{}_LocalTrainDelay_mu{}_beta{}_LocalIterNum{}_LocalBatchSize{}_Lambda{}_maxSpeed{}_noniid{}_SplitDict{}.png'.format(
        args.round_duration, args.mu_local_train, args.beta_local_train, args.local_iter, args.local_bs, args.Lambda, args.maxSpeed, args.non_iid, args.split_dict))
